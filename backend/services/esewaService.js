...
class EsewaService{
    static async createPaymentRequest(paymentData){
        try{
            ...
            const transactionUuid= `${Date.now()}-${uuidv4().substring(0,0)}`;
 ...
 // Create signed field names
      ...
      // Create signature message
            const signatureMessage= `total_amount=${totalAmount},transaction_uuid=${transactionUuid},
...
// Create transaction record
      ...
      // Create signature message for verification
                const signatureMessage= `transaction_code=${transaction_code},
                status=${status}, total_amount=${total_amount}, transaction_uuid=${transaction_uuid},
                product_code=${product_code}, signed_field_names=${signed_field_names}`;
      ...
      // Verify signature
        ...
        static async checkPaymentStatus(transactionUuid){
            try {
                ...
                const statusUrl=`${esewaConfig.statusUrl} ? 
                product_code=${transaction.product_code} & total_amount=${transaction.total_amount} & transaction_uuid=${transactionUuid}`;
...
// Update transaction status
        ...
        static async getTransactionById(transactionUuid){
            try {
                ...
                .populate('technician_id', 'name email phone');
